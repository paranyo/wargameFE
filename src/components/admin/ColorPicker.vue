...
export default {
  ...
  computed: {
		...
		gradientH () {
			...
			return { backgroundImage: "lineaer-gradient(to right, " + stops.join(", ") + ")" }
		},
    gradientS() {
      var stops = [];
      var c;
  ...
  mounted() {
		...
		let a = this.setColor.replace("%", '').replace('%', '').slice(4).slice(0, -1).split(',')
		this.h = a[0]
		this.s = a[1]
		this.l = a[2] * 2
	},
  methods: {
    ...
    hide()	 {
			this.UPDATE_SETTING({ name: 'backColor', value: this.setColor }).then(() => {
				this.SET_BACK_COLOR(this.setColor)
				alert('설정 완료')
			})
			this.isVisible = false 
		},
    toggle() { this.isVisible = !this.isVisible; },
		updateColor(color) { this.setColor = color },
    hsb2hsl(h, s, b) {
			var hsl = { h: h };
      hsl.l = (2 - s) * b;
      hsl.s = s * b;
      if (hsl.l <= 1 && hsl.l > 0)  hsl.s /= hsl.l;
      else													hsl.s /= 2 - hsl.l;
      hsl.l /= 2
      if (hsl.s > 1) hsl.s = 1;
      if (!hsl.s > 0) hsl.s = 0
